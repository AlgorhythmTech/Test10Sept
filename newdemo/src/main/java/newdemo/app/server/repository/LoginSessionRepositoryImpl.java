...
import newdemo.app.shared.authentication.LoginSession;
...
public class LoginSessionRepositoryImpl extends SearchInterfaceImpl implements LoginSessionRepositoryInterface, LoginSessionRepository<LoginSession> {
    ...
    public void saveSession(UserAuthentication userAuthentication, String appSessionId, String appServerSessinId, Timestamp loginTime, Timestamp logOutTime, String ClientIPAddress, Long ClientIPAddressInt, int ClientNetworkAddress, String ClientBrowser, String SessionData) throws SpartanPersistenceException {
        newdemo.app.shared.authentication.LoginSession loginSession = new newdemo.app.shared.authentication.LoginSession();
    ...
    public LoginSessionInterface findById(String AppSessionId) throws SpartanPersistenceException, Exception {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            javax.persistence.Query query = emanager.createQuery("select l from LoginSession l where l.appServerSessionId=:AppServerSessionId");
            query.setParameter("AppServerSessionId", AppSessionId);
    ...
    @Override
    @Transactional
    public void updateLastAccessTime(String userId, Timestamp timestamp) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            javax.persistence.Query query = emanager.createQuery("update LoginSession l set l.lastAccessTime=:lastAccessTime where l.userId=:userId");
