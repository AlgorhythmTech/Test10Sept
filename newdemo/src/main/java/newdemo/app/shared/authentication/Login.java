package newdemo.app.shared.authentication;
...
@Entity
@NamedNativeQuery(name = "complexQuery", query = "SELECT login.appCreatorId FROM ast_Login_T AS login,`ast_LJOY8TGFKDNIK6QYFRRJQ_CoreContacts_T` AS corecontact,`ast_LJOY8TGFKDNIK6QYFRRJQ_User_T` AS userinfo, `ast_LJOY8TGFKDNIK6QYFRRJQ_UserData_TP` AS userdata WHERE loginId=? AND login.`contactId`=corecontact.`contactId` AND login.`userId`=userinfo.`userId` AND userinfo.`userId`=userdata.userId")
...
public class Login implements Serializable, CommonEntityInterface, UserAuthentication, Comparator<Login> {
    ...
    public boolean equals(Object obj) {
        try {
            newdemo.app.shared.authentication.Login other = (newdemo.app.shared.authentication.Login) obj;
            if (loginPk == null) {
                return false;
